    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Real Estate Social Media Intelligence</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Accounts Tracked" value={stats?.accountsTracked || 0} icon={Users} description="Active competitor accounts" />
        <StatCard title="Total Followers" value={stats?.totalFollowers || 0} icon={TrendingUp} description="Combined audience" />
        <StatCard title="Posts (7 days)" value={stats?.recentPostsCount || 0} icon={FileText} description="Recent content published" />
        <StatCard title="Avg Engagement" value={`${(stats?.avgEngagementRate || 0).toFixed(2)}%`} icon={Heart} description="Likes + comments / followers" />
      </div>

      <div className="grid gap-4 md:grid-cols-3">
        <StatCard title="Total Likes" value={stats?.totalLikes || 0} icon={Heart} description="Last 7 days" />
        <StatCard title="Total Comments" value={stats?.totalComments || 0} icon={MessageCircle} description="Last 7 days" />
        <StatCard title="Total Views" value={stats?.totalViews || 0} icon={Eye} description="Last 7 days" />
      </div>

      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold">Competitor Rankings</h2>
          <MultiSelect options={TIME_PERIOD_OPTIONS} selected={competitorDays} onChange={setCompetitorDays} placeholder="Time Period" />
        </div>
        <CompetitorTable data={competitors || []} title={`Competitor Rankings (${TIME_PERIOD_OPTIONS.find(o => o.value === competitorDays[0])?.label || "Last 30 Days"})`} />
      </div>
    </div>
  );
}

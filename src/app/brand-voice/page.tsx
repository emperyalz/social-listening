...
function VoiceConsistencyHero({
  ...
  return (
    <GlassCard
      className="p-8"
      gradient={`linear-gradient(135deg, ${getScoreGradient(overallConsistency)} 0%, rgba(15, 23, 42, 0.9) 50%)`}
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="grid grid-cols-4 gap-6">
            <div>
              <span className="text-slate-400 block text-sm">Tone Match</span>
              <span className={`font-bold text-xl ${getScoreColor(toneConsistency)}`}>
                {toneConsistency}%
              </span>
            </div>
            <div>
              <span className="text-slate-400 block text-sm">Formality Match</span>
              <span className={`font-bold text-xl ${getScoreColor(formalityConsistency)}`}>
                {formalityConsistency}%
              </span>
            </div>
            <div>
              <span className="text-slate-400 block text-sm">Dominant Tone</span>
              <span className="font-bold text-xl text-violet-400">{dominantTone}</span>
            </div>
            <div>
              <span className="text-slate-400 block text-sm">Formality</span>
      ...
      <div className="h-[280px]">
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={radarData}>
            ...
            <PolarAngleAxis dataKey="trait" stroke="#64748b" fontSize={11} />
          ...
          <div key={tone.trait}>
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white font-medium">{tone.trait}</span>
              </div>
              <span className="text-slate-400 text-sm">{tone.score} signals</span>
            </div>
  ...
  return (
    <GlassCard className="p-6">
      ...
      <div className="relative h-8 bg-gradient-to-r from-pink-500 via-yellow-500 via-slate-500 via-blue-500 to-indigo-500 rounded-full mb-4">
        ...
        <div
          className="absolute top-1/2 -translate-y-1/2 w-6 h-6 bg-white rounded-full border-4 border-slate-900 shadow-lg transition-all duration-500"
      ...
      <div className="flex justify-between text-xs text-slate-500 mb-6">
        {gaugePositions.map((pos) => (
          <span key={pos.label} className={level === pos.label ? "text-white font-medium" : ""}>
            {pos.label}
          </span>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between p-4 bg-slate-800/30 rounded-xl">
        <div>
          <span className="text-slate-400 text-sm block">Current Level</span>
          <span className="text-white font-bold text-xl">{level}</span>
        </div>
        <div className="text-right">
          <span className="text-slate-400 text-sm block">Score</span>
    ...
    <GlassCard className="p-6">
      ...
      <div className="grid grid-cols-3 gap-2 mt-4">
        {scores.slice(0, 5).map((score, i) => (
          <div key={score.trait} className="flex items-center gap-2 text-sm">
            <div
              className="w-3 h-3 rounded"
              style={{ backgroundColor: PERSONALITY_COLORS[i % PERSONALITY_COLORS.length] }}
            />
            <span className="text-slate-400 truncate">{score.trait}</span>
          </div>
        ))}
      </div>
    </GlassCard>
          ...
          {data.map((platform) => (
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  <span className="text-slate-500 block">Dominant Tone</span>
                  <span className="text-violet-400 font-medium">{platform.dominantTone}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Formality</span>
                  <span className="text-cyan-400 font-medium">{platform.formalityLevel}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Avg Words/Sentence</span>
                  <span className="text-emerald-400 font-medium">{platform.avgSentenceLength}</span>
                </div>
              </div>
            </div>
          ))}
...
function TargetVoiceConfig({ config }: { config: TargetVoice }) {
  return (
    <GlassCard className="p-6">
      ...
      <div className="space-y-4">
        <div>
          <span className="text-slate-400 text-sm block mb-2">Primary Tone</span>
          <span className="px-4 py-2 bg-violet-500/20 text-violet-300 rounded-full text-sm font-medium">
            {config.primaryTone}
          </span>
        </div>
        
        <div>
          <span className="text-slate-400 text-sm block mb-2">Secondary Tone</span>
        ...
        <div>
          <span className="text-slate-400 text-sm block mb-2">Formality Target</span>
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-medium">{config.formalityTarget}%</span>
          </div>
        </div>
        
        <div>
          <span className="text-slate-400 text-sm block mb-2">Personality Focus</span>
          ...
          {filteredData.map((category) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-slate-400 text-sm">{category.count} uses</span>
              </div>
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-950">
        <div className="text-center">
          ...
          <p className="text-slate-400">Analyzing Brand Voice...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mb-8">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-slate-400 mt-1">Tone, Style & Voice Consistency Analysis</p>
          </div>
      ...
      <section className="pt-6 border-t border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-4 text-sm text-slate-500">
          <div className="flex flex-wrap items-center gap-4 md:gap-6">
            <span>
              <strong className="text-slate-300">{voiceData.summary.totalPostsAnalyzed}</strong>{" "}
              posts analyzed
            </span>
            <span>
              <strong className="text-slate-300">{voiceData.summary.totalCommentsAnalyzed}</strong>{" "}
              comments analyzed
            </span>
            <span>
              Avg <strong className="text-slate-300">{voiceData.summary.avgSentenceLength}</strong>{" "}
              words/sentence
            </span>
            <span>
              Period: <strong className="text-slate-300">{voiceData.periodDays} days</strong>
            </span>
          </div>
    ...
    <Suspense
      fallback={
        <div className="flex items-center justify-center min-h-screen bg-slate-950">
          <div className="text-center">
            <div className="w-12 h-12 border-4 border-violet-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" />
            <p className="text-slate-400">Loading Brand Voice Analysis...</p>
          </div>
        </div>
      }
    >
